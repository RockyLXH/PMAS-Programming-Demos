...
//
	...
	cConn.RegisterEventCallback(MMCPP_STOP_ON_LIMIT, (void*)stop_on_limit_callback);
	//
	...
	cAxes[0].PowerOn(MC_BUFFERED_MODE);
	cAxes[1].PowerOn(MC_BUFFERED_MODE);
	cAxes[2].PowerOn(MC_BUFFERED_MODE);
	sleep(1);
	cAxes[0].SetPosition(10100, 0);
	cAxes[2].MoveRelative(100000000, 10000);
	while(1);


//	MMC_GEARIN_IN gear_in;
//	gear_in.dbAcceleration = 100000;
//	gear_in.dbDeceleration = 100000;
//	gear_in.dbJerk = 1000000;
//	gear_in.eBufferMode = MC_ABORTING_MODE;
//	gear_in.eMasterValueSource = eECAM_ACTUAL_VALUE;
//	gear_in.eSyncMode = eSLOW_DOWN;
//	gear_in.iRatioDenominator = 1;
//	gear_in.iRatioNumerator = 1;
//	gear_in.usMaster = cAxes[0].GetRef();
//	gear_in.ucExecute = 1;
//	MMC_GEARIN_OUT gear_out;
//	MMC_GearInCmd(gConnHndl, cAxes[1].GetRef(), &gear_in, &gear_out);


//	cAxes[1].GearInPos(cAxes[0].GetRef(), 1000, 2000, 500);
...
void stop_on_limit_callback(unsigned short usAxisRef)
{
	cout << "the axis " << usAxisRef << " is on F/R limitation" << endl;
	cAxes[usAxisRef].Stop();
}
